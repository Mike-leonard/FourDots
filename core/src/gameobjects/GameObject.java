...
public class GameObject {
    ...
    public GameObject(final GameWorld world, float x, float y, float width, float height,
        ...
        cbStart = new TweenCallback() {
            @Override
            public void onEvent(int type, BaseTween<?> source) {
                world.getBall().setVelocity(new Vector2(0, Settings.INITAL_BALL_SPEED));
                world.getBall().setAcceleration(new Vector2(0, Settings.BALL_ACCELERATION));
            }
        };
    }
    ...
    public void updateEffects() {
        ...
        if (shadow) {
            ...
            shadowSprite.setPosition(sprite.getX(),
                    sprite.getY() - 5);
    ...
    public void effectYStart(float from, float to, float duration, float delay) {
        position.y = from;
        Tween.to(position, VectorAccessor.VERTICAL, duration).target(to).delay(delay)
                .ease(TweenEquations.easeInOutSine).setCallback(cbStart).setCallbackTriggers(
